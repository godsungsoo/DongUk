...
SELECT USER_NO, USER_NAME, ACCOUNT_NO, BALANCE, OPEN_DATE, TRANS_DATE, PHONE FROM BANKMANAGER JOIN TRANSACTION USING (USER_NO) JOIN ACCOUNT USING (ACCOUNT_NO) WHERE USER_NAME = '김동욱';

INSERT INTO TRANSACTION VALUES(1,'02-100', DEFAULT, 1, ' ',2000,0,3000);
commit;

select trans_date, max(account_no)
from transaction
join bankmanager using (user_no)
where user_name = '김동욱'
group by trans_date;
--같은 계좌 중에서 trans_date 순서로 정렬 후 1등을 뽑아냄
